 ...
 *@author: monster_zee
 *@date:   2019.12.8  根据鼠标事件，实现内部部件发生平移的容器
 *@update:  2019.12.25　加入平移动画效果
 *@update:  2019.12.31    提供外界设置容器当前页的接口
...
TranslationWidget::TranslationWidget(QList<QWidget *> &list, QWidget *parent) :
    QWidget(parent),posX(0),posY(0),miniumMoveX(parent->width()/3),maximumMoveX(parent->width()/8)
{
    //初始化鼠标横坐标相对位置以及移动距离
    cursorX=0;
    moveX=0;
    ...
    if(pageNum<1)
    {
        qWarning()<<"No Widget In TranslationWidget!";
        return ;
    }
